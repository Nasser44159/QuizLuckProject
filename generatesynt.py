...
def remove_action_buttons_and_section(content):
    ...
    # Supprimer le conteneur du timer (<div id="timer-container">) et son commentaire
    content = re.sub(r'<!-- Conteneur du timer -->\s*<div id="timer-container">.*?</div>', '', content, flags=re.DOTALL)

    # Supprimer le fond flouté (blur-background)
    content = re.sub(r'<!-- Fond flouté pour l\'effet d\'arrière-plan -->\s*<div class="blur-background" .*?</div>', '', content, flags=re.DOTALL)

    # Supprimer le popup de résultat (result-popup)
    content = re.sub(r'<!-- Popup de résultat -->\s*<div class="result-popup" .*?</div>', '', content, flags=re.DOTALL)
